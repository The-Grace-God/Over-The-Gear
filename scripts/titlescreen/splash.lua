...
local splash7Logo = gfx.CreateSkinImage("titlescreen/splash/2.PNG", 0)
local splash7LogoWidth, splash7LogoHeight = gfx.ImageSize(splash7Logo)

local splash8BgColor = {0, 0, 0}
local splash8Logo = gfx.CreateSkinImage("titlescreen/splash/3.PNG", 0)
local splash8LogoWidth, splash8LogoHeight = gfx.ImageSize(splash8Logo)

local splash9BgColor = {0, 0, 0}
local splash9Logo = gfx.CreateSkinImage("titlescreen/splash/3.PNG", 0)
local splash9LogoWidth, splash9LogoHeight = gfx.ImageSize(splash9Logo)
...
local function splash9(deltaTime)
    local splash9LogoXOffset = (Dim.design.width - splash9LogoWidth) / 2
    local splash9LogoYOffset = (Dim.design.height - splash9LogoHeight) / 2

    calcFade(splash8Duration)

    gfx.BeginPath()
    gfx.Rect(0, 0, Dim.design.width, Dim.design.height)
    gfx.FillColor(splash9BgColor[1], splash9BgColor[2], splash9BgColor[3], fadeAlpha)
    gfx.Fill()

    gfx.BeginPath()
    gfx.ImageRect(splash9LogoXOffset, splash9LogoYOffset, splash9LogoWidth, splash9LogoHeight, splash9Logo, fadeAlpha / 255, 0)
...
function render(deltaTime)
    ...
    elseif splashState == "splash6" then
        splash6(deltaTime)
	elseif splashState == "splash7" then
        splash7(deltaTime)
	elseif splashState == "splash8" then
        splash8(deltaTime)
	elseif splashState == "splash9" then
        splash9(deltaTime)
        reset()
        return {
            eventType = "switch",
