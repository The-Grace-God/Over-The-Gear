...
function BootPage.new(o)
    ...
    this:addField(ServiceField.new{posX = 32, posY = 32, label = "KFC:J:G:A:2022101800", value = ""})
    this:addField(ServiceField.new{posX = 64, posY = 64, label = "SOUND VOLTEX EXCEED GEAR STARTUP...", value = ""})

    local valueOffX = 220
	this._securityTestField = SelfTestField.new{label = "MAIN I/O", VALUE_OFFSETX = valueOffX}
    this._securityTestField.checkTask = function(obj)
	return SelfTestStatusEnum.OK
    end
    this._securityTestField.onStatusChange = function(status)
        if status == SelfTestStatusEnum.OK then
            this._icTestField:activate()
        end
    end
	
    this._mainIoTestField = SelfTestField.new{label = "SECURITY", VALUE_OFFSETX = valueOffX}
    this._mainIoTestField.checkTask = function(obj)
        return SelfTestStatusEnum.OK
    end
    this._mainIoTestField.onStatusChange = function(status)
        if status == SelfTestStatusEnum.OK then
            this._securityTestField:activate()
        end
    end
	
	this._cameraTestField = SelfTestField.new{label = "RGB CAMERA", VALUE_OFFSETX = valueOffX}
    this._cameraTestField.checkTask = function(obj)
	return SelfTestStatusEnum.OK
    end
    this._cameraTestField.onStatusChange = function(status)
        if status == SelfTestStatusEnum.OK then
            this._skinConfigTestField:activate()
        end
    end

	this._icTestField = SelfTestField.new{label = "IC CARD READER", VALUE_OFFSETX = valueOffX}
    this._icTestField.checkTask = function(obj)
	return SelfTestStatusEnum.OK
    end
    this._icTestField.onStatusChange = function(status)
        if status == SelfTestStatusEnum.OK then
            this._cameraTestField:activate()
        end
    end

    this._skinConfigTestField = SelfTestField.new{label = "BACKUP DATA", VALUE_OFFSETX = valueOffX}
	...
	list:addField(this._securityTestField)
	list:addField(this._icTestField)
	list:addField(this._cameraTestField)
